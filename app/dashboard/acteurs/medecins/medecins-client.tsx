...
export type MedecinListItem = { id: string; idNational: string; idFederal: string; nomComplet: string; sexe: string; dateNaissance: string; federation: string; specialite: string; statut: string; avatar: string | null }
...
export default function MedecinsClient({ medecins, organisations, specialites }: { medecins: MedecinListItem[]; organisations: EntityOption[]; specialites: SpecialtyOption[] }) {
  ...
  async function save() {
    ...
    try {
      ...
      settled.some((item) => item.status === "rejected") ? toast.warning("Médecin créé, mais un média n’a pas pu être envoyé.") : toast.success("Médecin ajouté.")
  ...
  return <div className="min-h-screen">
    ...
    <div className="space-y-6 p-6">
      <div className="flex justify-between gap-4"><div className="relative w-full max-w-md"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input className="pl-9" placeholder="Rechercher un médecin..." value={search} onChange={(e) => setSearch(e.target.value)} /></div><Button onClick={() => setOpen(true)}><Plus className="mr-2 h-4 w-4" />Ajouter un médecin</Button></div>
