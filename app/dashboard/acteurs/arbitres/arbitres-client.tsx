...
export type ArbitreListItem = { id: string; idNational: string; idFederal: string; nomComplet: string; sexe: string; federation: string; grade: string; statut: string; avatar: string | null }
...
export default function ArbitresClient({ arbitres, federations, grades }: { arbitres: ArbitreListItem[]; federations: FederationOption[]; grades: GradeOption[] }) {
  ...
  const filtered = useMemo(() => { const q = search.trim().toLocaleLowerCase("fr"); const matching = !q ? arbitres : arbitres.filter((item) => [item.idNational, item.idFederal, item.nomComplet, item.federation, item.grade, item.statut].some((v) => v.toLocaleLowerCase("fr").includes(q))); return [...matching].sort((a, b) => a.nomComplet.localeCompare(b.nomComplet, "fr", { sensitivity: "base" })) }, [arbitres, search])
