...
export default async function ArbitreDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const arbitre: ArbitreDetail = { id: r.id_arbitre_coc, idFederation: r.id_federation || "", idFederal: r.id_arbitre_federation || "", idNational: r.id_national || "", idInternational: r.id_international || "", nomComplet: r.nom_complet || "", sexe: r.id_sexe || r.nom_sexe || "", dateNaissance: r.date_de_naissance || "", lieuNaissance: r.lieu_de_naissance || "", nationalite: r.nationalite || "", federation: linkedFederation?.sigle || linkedFederation?.nom || r.nom_federation || "", idGrade: r.id_grade_arbitre || r.id_grade || "", grade: linkedGrade?.nom_grade || r.nom_grade || "", dateAffiliation: r.date_affiliation || "", telephone: r.telephone || "", email: r.email || "", adresse: r.adresse || "", numeroPasseport: r.numero_passeport || "", dateDelivrancePasseport: r.date_de_delivrance_passeport || "", dateExpirationPasseport: r.date_expiration_passeport || "", statut: r.statut?.toLowerCase() === "inactif" ? "inactif" : "actif", avatarUrl: r.avatar_drive_url || null, urlPasseport: r.passeport_drive_url || null }
