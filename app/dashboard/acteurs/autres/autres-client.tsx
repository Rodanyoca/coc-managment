...
export default function AutresClient({ actors, references, canWrite, loadError = false }: { actors: OtherActorView[]; references: OtherActorReferences; canWrite: boolean; loadError?: boolean }) {
  ...
  const filtered = useMemo(() => actors.filter((actor) => {
    ...
    const matchesQuery = !needle || [actor.nom_complet, actor.type_autre_acteur, actor.entiteNom, actor.entiteSigle, actor.federationNom, actor.federationSigle].some((value) => value.toLocaleLowerCase("fr").includes(needle))
  ...
  return <div className="min-h-screen min-w-0 overflow-x-hidden"><Header title="Autres acteurs" subtitle="Responsables, contacts, partenaires et représentants d’entités" />
    <main className="min-w-0 space-y-5 p-4 md:p-6">
      ...
      </div>{canWrite && <Button asChild><Link href="/dashboard/acteurs/autres/nouveau"><Plus className="h-4 w-4" />Ajouter un autre acteur</Link></Button>}</div>
      <Card className="min-w-0 overflow-hidden"><CardContent className="p-0"><div className="hidden lg:block"><Table><TableHeader><TableRow><TableHead>Identité</TableHead><TableHead>Fonction / qualité</TableHead><TableHead>Rattachement</TableHead><TableHead>Téléphone</TableHead><TableHead>Adresse électronique</TableHead><TableHead>Statut</TableHead><TableHead className="w-16 text-right">Action</TableHead></TableRow></TableHeader><TableBody>{visible.map((actor) => <TableRow key={actor.id_autre_acteur_coc}><TableCell className="max-w-52 whitespace-normal font-medium">{actor.nom_complet || "Non renseigné"}</TableCell><TableCell className="max-w-48 whitespace-normal">{actor.type_autre_acteur || "Non renseigné"}</TableCell><TableCell className="max-w-52 whitespace-normal">{affiliation(actor)}</TableCell><TableCell>{actor.telephone || "—"}</TableCell><TableCell className="max-w-56 break-all whitespace-normal">{actor.email || "—"}</TableCell><TableCell><Badge variant="secondary">{actor.statut || "Non renseigné"}</Badge></TableCell><TableCell className="text-right"><DetailLink actor={actor} /></TableCell></TableRow>)}{!visible.length && <TableRow><TableCell colSpan={7} className="h-28 text-center text-muted-foreground">{actors.length ? "Aucun résultat pour ces critères." : "Aucun autre acteur enregistré."}</TableCell></TableRow>}</TableBody></Table></div>
        <div className="grid min-w-0 gap-3 p-3 sm:grid-cols-2 lg:hidden">{visible.map((actor) => <article key={actor.id_autre_acteur_coc} className="min-w-0 rounded-lg border p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h2 className="break-words font-semibold">{actor.nom_complet || "Non renseigné"}</h2><p className="break-words text-sm text-muted-foreground">{actor.type_autre_acteur || "Fonction non renseignée"}</p></div><DetailLink actor={actor} /></div><div className="mt-3 space-y-1 text-sm"><p className="break-words"><span className="text-muted-foreground">Rattachement : </span>{affiliation(actor)}</p>{actor.telephone && <p className="break-all">{actor.telephone}</p>}{actor.email && <p className="break-all">{actor.email}</p>}<Badge variant="secondary" className="mt-2">{actor.statut || "Non renseigné"}</Badge></div></article>)}{!visible.length && <p className="py-10 text-center text-sm text-muted-foreground sm:col-span-2">{actors.length ? "Aucun résultat pour ces critères." : "Aucun autre acteur enregistré."}</p>}</div>
