...
function ActivityForm({ form, setForm, refs, selectedEntities = [], setSelectedEntities }: { form: typeof empty; setForm: React.Dispatch<React.SetStateAction<typeof empty>>; refs: ActivityReferences; selectedEntities?: string[]; setSelectedEntities?: React.Dispatch<React.SetStateAction<string[]>> }) {
  ...
  return <div className="grid gap-4 px-4 sm:grid-cols-2">
    ...
    <div className="space-y-2 sm:col-span-2"><Label>Entités participantes</Label><Select onValueChange={(id) => setSelectedEntities?.((current) => current.includes(id) ? current : [...current, id])}><SelectTrigger><SelectValue placeholder="Ajouter une entité (facultatif)" /></SelectTrigger><SelectContent>{refs.entites.filter((item) => item.id !== form.id_entite_organisatrice && !selectedEntities.includes(item.id)).map((item) => <SelectItem key={item.id} value={item.id}>{item.secondary ? `${item.secondary} — ` : ""}{item.label}</SelectItem>)}</SelectContent></Select><div className="flex flex-wrap gap-2">{selectedEntities.map((id) => <Badge key={id} variant="secondary">{refs.entites.find((item) => item.id === id)?.secondary || id}<button type="button" className="ml-2" onClick={() => setSelectedEntities?.((current) => current.filter((item) => item !== id))} aria-label="Retirer">×</button></Badge>)}</div></div>
  </div>
}
...
export default function Client({ initialRows, references, loadError }: { initialRows: Activity[]; references: ActivityReferences; loadError?: string }) {
  const router = useRouter(), [open, setOpen] = useState(false), [saving, setSaving] = useState(false), [form, setForm] = useState(empty), [selected, setSelected] = useState<string[]>([]), [query, setQuery] = useState("")
  const rows = useMemo(() => initialRows.filter((item) => [item.nom_activite, item.titre_public, item.lieu, item.ville].join(" ").toLowerCase().includes(query.toLowerCase())).sort((a, b) => b.date_debut.localeCompare(a.date_debut)), [initialRows, query])
  ...
  async function save() { setSaving(true); try { const response = await fetch("/api/activites", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ row: form, entityIds: selected }) }); const result = await response.json(); if (!response.ok) throw new Error(result.error); if (result.partialError) toast.warning(result.partialError); else toast.success("Activité créée."); setOpen(false); setForm(empty); setSelected([]); router.refresh() } catch (error) { toast.error(error instanceof Error ? error.message : String(error)) } finally { setSaving(false) } }
  const counters: [string, number][] = [["Total", initialRows.length], ["Planifiées", initialRows.filter((item) => item.statut_normalise === "PLANIFIE").length], ["En cours", initialRows.filter((item) => item.statut_normalise === "EN_COURS").length], ["Réalisées", initialRows.filter((item) => ["REALISE", "TERMINE"].includes(item.statut_normalise)).length], ["Annulées", initialRows.filter((item) => item.statut_normalise === "ANNULE").length]]
