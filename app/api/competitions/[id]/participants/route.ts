...
export async function GET(_:Request,context:Context){if(!(await canAccess("AUT-SPT","READ")))return NextResponse.json({error:"Accès refusé."},{status:403});const{id}=await context.params;return NextResponse.json({rows:await getAthleteParticipations({competitionId:id,fresh:true}),references:await getParticipationReferences()})}
