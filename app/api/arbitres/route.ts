...
async function validate(row: Record<string, string>, currentId = "") {
  ...
  const grade = row.id_grade ? grades.find((item) => item.id_grade === row.id_grade) : undefined
...
function enrich(row: Record<string, string>, context: { federation: { nom: string }; grade?: Record<string, string> }) {
  return { ...row, nom_federation: context.federation.nom, nom_grade: context.grade?.nom_grade || "", nom_sexe: row.id_sexe === "F" ? "Femme" : "Homme" }
}
