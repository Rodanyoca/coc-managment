...
export function GeneralSummarySection({ territorial, actors, activities, competitions, nationalTeams, documents }: {
  territorial?: TerritorialDashboardStats
  actors?: ActorsDashboardStats
  activities?: ActivitiesDashboardStats
  competitions?: CompetitionsDashboardStats
  nationalTeams?: NationalTeamsDashboardStats
  documents?: DocumentsDashboardStats
}) {
  return <section aria-labelledby="summary-title" className="space-y-4">
    ...
    <div className="grid overflow-hidden rounded-lg border border-border/60 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
      ...
      {activities && <div className="min-h-40 space-y-3 border-t p-6 lg:border-l lg:border-t-0"><p className="text-sm font-medium text-muted-foreground">Activités</p><p className="text-4xl font-semibold tracking-tight tabular-nums">{number.format(activities.totalActivities)}</p><p className="max-w-xs text-sm text-muted-foreground">Activités institutionnelles et sportives enregistrées</p></div>}
