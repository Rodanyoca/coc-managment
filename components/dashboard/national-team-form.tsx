...
export function NationalTeamForm({ value, onChange, references, saving, onSubmit, onCancel, editing = false }: { value: NationalTeamFormValue; onChange: (value: NationalTeamFormValue) => void; references: NationalTeamReferences; saving: boolean; onSubmit: () => void; onCancel?: () => void; editing?: boolean }) {
  ...
  const update = (key: keyof NationalTeamFormValue, next: string) => {
    if (key === "id_federation") {
      ...
      onChange({ ...value, id_federation: next, id_sport: selected?.parentId || "", id_discipline: "" })
      return
    }
  ...
  return <div className="space-y-6">
    <div className="grid gap-4 sm:grid-cols-2">
      ...
      <div className="space-y-2"><Label>Catégorie d’âge</Label><Select disabled={!references.ageCategoriesAvailable} value={value.id_categorie_age || "aucune"} onValueChange={(next) => update("id_categorie_age", next === "aucune" ? "" : next)}><SelectTrigger><SelectValue placeholder="Non renseignée" /></SelectTrigger><SelectContent><SelectItem value="aucune">Non renseignée</SelectItem>{references.ageCategories.map((item) => <SelectItem key={item.id} value={item.id}>{item.label}</SelectItem>)}</SelectContent></Select>{!references.ageCategoriesAvailable && <p className="text-xs text-amber-700">La feuille CATEGORIES_AGE existe, mais elle ne contient encore aucune valeur.</p>}</div>
