...
export async function getParticipatingUnits(competitionId?:string):Promise<ParticipatingUnit[]>{const [units,members,engagements,participants]=await Promise.all([getSheetRows({sheetName:"UNITES_PARTICIPANTES",spreadsheetId:getCompetitionsSpreadsheetId()}),getSheetRows({sheetName:"MEMBRES_UNITES_PARTICIPANTES",spreadsheetId:getCompetitionsSpreadsheetId()}),getCampaignEngagements(competitionId?{competitionId}:{}),getAthleteParticipations(competitionId?{competitionId}:{})]);const engagementIds=new Set(engagements.map(x=>x.id_engagement_campagne)),participantMap=new Map(participants.map(x=>[x.id_participation_acteur,x]));return units.filter(x=>engagementIds.has(x.id_engagement_campagne)).map(x=>{const memberIds=members.filter(m=>m.id_unite_participante===x.id_unite_participante).map(m=>m.id_participation_acteur),direct=x.id_participation_acteur?[x.id_participation_acteur]:[];return{id_unite_participante:clean(x.id_unite_participante),id_engagement_campagne:clean(x.id_engagement_campagne),type_unite:clean(x.type_unite) as "INDIVIDUEL"|"EQUIPE",id_participation_acteur:clean(x.id_participation_acteur),nom_unite:clean(x.nom_unite),observation:clean(x.observation),membres:memberIds,composition:[...direct,...memberIds].map(id=>participantMap.get(id)?.athlete_label||participantMap.get(id)?.actor_label||id)}}).filter(x=>x.id_unite_participante)}
