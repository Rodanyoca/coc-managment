...
export function validateMemberInput(input: Record<string, unknown>) {
  const row = { id_acteur_coc: clean(input.id_acteur_coc), id_type_acteur: clean(input.id_type_acteur).toUpperCase(), role_selection: clean(input.role_selection), date_debut: clean(input.date_debut), date_fin: clean(input.date_fin), statut: clean(input.statut).toUpperCase(), observations: clean(input.observations) }
  if (!row.id_acteur_coc || !ACTOR_TYPES.includes(row.id_type_acteur as ActorType) || !row.role_selection || !row.date_debut || !row.statut) throw new Error("Le type, l’acteur, le rôle, la date de début et le statut sont obligatoires.")
