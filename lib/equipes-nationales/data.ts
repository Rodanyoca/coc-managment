...
export async function createNationalTeamCampaign(teamId: string, input: Record<string, unknown>) {
  ...
  if (existing.some((item) => item.id_equipe_nationale === teamId && normalized(item.nom_campagne) === normalized(row.nom_campagne) && item.date_debut === row.date_debut)) throw new Error("Cette campagne existe déjà.")
...
export async function updateNationalTeamCampaign(teamId: string, id: string, input: Record<string, unknown>) {
  ...
  if (existing.some((item) => item.id_campagne !== id && normalized(item.nom_campagne) === normalized(row.nom_campagne) && item.date_debut === row.date_debut)) throw new Error("Cette campagne existe déjà.")
...
async function currentCampaign(teamId: string) {
  ...
  const matches = rows.filter((row) => row.id_equipe_nationale === teamId).sort((a, b) => b.date_debut.localeCompare(a.date_debut))
  return matches.find((row) => row.statut === "ACTIF") || matches[0]
}
