...
const ACTIVITY_SHEET_HEADERS=["id_activite","id_type_activite","id_entite_organisatrice","nom_activite","titre_public","resume","date_debut","date_fin","pays","ville","lieu","id_statut_activite","observation"] as const
const ENTITY_SHEET_HEADERS=["id_activite_entite","id_activite","id_entite","id_role_entite_activite","observation"] as const
const PARTICIPANT_SHEET_HEADERS=["id_participation","id_activite","id_acteur_coc","id_type_acteur","id_entite_representee","id_role_participation","observation"] as const
const activity=(row:Record<string,string>)=>({...map(row,ACTIVITY_HEADERS),statut:clean(row.id_statut_activite),observations:clean(row.observation),statut_normalise:normalizeStatus(row.id_statut_activite)}) as Activity
const entity=(row:Record<string,string>)=>({...map(row,ACTIVITY_ENTITY_HEADERS),role_entite:clean(row.id_role_entite_activite),statut_participation:"",observations:clean(row.observation)}) as ActivityEntity
const participant=(row:Record<string,string>)=>({...map(row,PARTICIPANT_HEADERS),role_activite:clean(row.id_role_participation),statut_participation:""}) as ActivityParticipant
...
const activitySheetRow=(row:Record<string,string>):Record<string,string>=>({...row,id_statut_activite:row.statut,observation:row.observations})
...
export async function saveActivity(input:Record<string,unknown>,id?:string){const row=map(input as Record<string,string>,ACTIVITY_HEADERS.filter(x=>x!=="id_activite")) as Record<string,string>;await validateActivity(row);const rows=await getActivities();const physical=activitySheetRow(row);if(id){if(!rows.some(x=>x.id_activite===id))throw new Error("Activité introuvable.");await updateSheetCells({sheetName:"ACTIVITES",spreadsheetId:getActivitesSpreadsheetId(),idColumn:"id_activite",idValue:id,updates:ACTIVITY_SHEET_HEADERS.filter(x=>x!=="id_activite").map(column=>({column,value:physical[column]}))});return{...row,id_activite:id}}const created={...row,id_activite:nextId(rows,"id_activite","ACT")};await appendSheetRow({sheetName:"ACTIVITES",spreadsheetId:getActivitesSpreadsheetId(),row:activitySheetRow(created)});return created}
...
const actorSheets: Record<ActorType, { sheetName: string; idKey: string }> = {
  ATHLETE: { sheetName: ACTOR_SHEETS.ATHLETE, idKey: "id_athlete_coc" },
  COACH: { sheetName: ACTOR_SHEETS.COACH, idKey: "id_coach_coc" },
